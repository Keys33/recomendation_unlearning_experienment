...
class RecEraser_LightGCN(object):
    ...
    def train_single_model(self, local_num):
        ...
        u_g_embeddings = tf.nn.embedding_lookup(params=ua_embeddings, ids=self.users)
        pos_i_g_embeddings = tf.nn.embedding_lookup(params=ia_embeddings, ids=self.pos_items)
        neg_i_g_embeddings = tf.nn.embedding_lookup(params=ia_embeddings, ids=self.neg_items)

        u_g_embeddings_pre = tf.nn.embedding_lookup(params=self.weights['user_embedding'][:, local_num], ids=self.users)
        pos_i_g_embeddings_pre = tf.nn.embedding_lookup(params=self.weights['item_embedding'][:, local_num], ids=self.pos_items)
        neg_i_g_embeddings_pre = tf.nn.embedding_lookup(params=self.weights['item_embedding'][:, local_num], ids=self.neg_items)
    ...
    def attention_based_agg(self, embs, flag):
        if flag == 0:
            ...
            embs_w = tf.compat.v1.div(embs_w, tf.reduce_sum(input_tensor=embs_w, axis=1, keepdims=True))
        else:
            embs_w = tf.exp(
                tf.einsum('abc,ck->abk', tf.nn.relu(
        ...
        agg_emb = tf.reduce_sum(input_tensor=tf.multiply(embs_w, embs), axis=1)
    ...
    def train_agg_model(self):
        ...
        ua_embeddings = tf.transpose(a=ua_embeddings, perm=[1, 0, 2])
        ia_embeddings = tf.transpose(a=ia_embeddings, perm=[1, 0, 2])
        ...
        u_e_drop = tf.nn.dropout(u_e, rate=1 - (self.dropout_keep_prob))
    ...
    def train_agg_model2(self):
        ...
        u_drop = tf.nn.dropout(u_g_embeddings, rate=1 - (self.dropout_keep_prob))
    ...
    def _init_weights(self):
        all_weights = dict()
        initializer = tf.compat.v1.random_normal_initializer(stddev=0.01)  # tf.contrib.layers.xavier_initializer()
        ...
        all_weights['item_embedding'] = tf.Variable(initializer([self.n_items, self.num_local, self.emb_dim]),
                                                    name='item_embedding')
        print(float(tf.sqrt(
                tf.compat.v1.div(2.0, self.attention_size + self.emb_dim))))

        # user attention
        all_weights['WA'] = tf.Variable(
            tf.compat.v1.truncated_normal(shape=[self.emb_dim, self.attention_size], mean=0, stddev=int(tf.sqrt(
                tf.compat.v1.div(2.0, self.attention_size + self.emb_dim)))), name='WA')
